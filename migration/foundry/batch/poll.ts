...
function mapFoundryStatus(
  foundryStatus: string,
): BatchJobStatus {
  switch (foundryStatus) {
    case "completed":
      return "completed";
    case "failed":
    case "cancelled":
...
export async function pollBatch(
  ...
  if (!job.foundry_job_id)
    throw new Error(`[batch] Job has no foundry_job_id: ${jobId}`);

  const response = await client.pollBatchJob(job.foundry_job_id);
  const status = mapFoundryStatus(response.status);
