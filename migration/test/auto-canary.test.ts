...
if (process.env.AGENT_PROVIDER_BASE_URL !== "https://private-relay.example/v1") process.exit(10);
if (process.env.AGENT_PROVIDER_API_KEY_ENV !== "EXAMPLE_PRIVATE_API_KEY") process.exit(11);
...
`, "utf8");
    const cfg: GuildConfig & { guildRoot: string; configPath: string; selectedProfile: string } = {
      ...
      model: { ...DEFAULT_GUILD_CONFIG.model, base_url: "https://private-relay.example/v1", api_key_env: "EXAMPLE_PRIVATE_API_KEY" },
    ...
    await withEnv({ EXAMPLE_PRIVATE_API_KEY: "secret-value-never-print", AGENT_CMD: script }, async () => {
...
if (process.env.AGENT_PROVIDER_BASE_URL !== "https://example-private.invalid/v1") process.exit(25);
if (process.env.AGENT_PROVIDER_API_KEY_ENV !== "EXAMPLE_PRIVATE_API_KEY") process.exit(26);
...
`, "utf8");
    ...
    await withEnv({
      ...
      EXAMPLE_PRIVATE_API_KEY: undefined,
    }, async () => {
  ...
  try {
    ...
    await assert.rejects(
      ...
      (error: Error) => {
        ...
        assert.doesNotMatch(error.message, /EXAMPLE_PRIVATE_API_KEY is missing/);
        return true;
      },
    );
  } finally {
