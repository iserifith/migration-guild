...
test("resolveAgentLaunch reports the harness, provider, and model a run will actually use", () => {
  ...
  try {
    const resolved = resolveAgentLaunch({
      config: config(),
      root,
      env: { EXAMPLE_PRIVATE_API_KEY: CREDENTIAL_VALUE },
    });
    ...
    assert.equal(resolved.providerBaseUrl, "https://example-private.invalid/v1");
...
test("credentialEnv carries the variable name only — the value never enters the resolved object", () => {
  ...
  try {
    ...
    assert.equal(resolved.credentialEnv, "EXAMPLE_PRIVATE_API_KEY");
...
test("a model or base URL differing from project configuration is reported as a divergence", () => {
  ...
  try {
    const resolved = resolveAgentLaunch({
      config: config(),
      root,
      env: { AGENT_PROVIDER_BASE_URL: "https://ambient.example/v1", EXAMPLE_PRIVATE_API_KEY: CREDENTIAL_VALUE },
      model: "pvt/grok-4.5",
    });
    ...
    assert.equal(modelDivergence.resolvedValue, "pvt/grok-4.5");

    const urlDivergence = resolved.divergences.find((d) => d.setting === "model.base_url");
    assert.ok(urlDivergence);
    assert.equal(urlDivergence.declaredValue, "https://example-private.invalid/v1");
...
test("agentEnv is the environment the agent process receives, including caller-supplied run variables", () => {
  ...
  try {
    const resolved = resolveAgentLaunch({
      config: config(),
      root,
      env: { PATH: "/usr/bin", EXAMPLE_PRIVATE_API_KEY: CREDENTIAL_VALUE },
      extraEnv: { GUILDCTL_RUN_ID: "run-1234" },
    });

    assert.equal(resolved.agentEnv["AGENT_PROVIDER_BASE_URL"], "https://example-private.invalid/v1");
    assert.equal(resolved.agentEnv["AGENT_PROVIDER_API_KEY_ENV"], "EXAMPLE_PRIVATE_API_KEY");
...
`, "utf8");
    ...
    assert.equal(agentEnv["AGENT_PROVIDER_BASE_URL"], "https://example-private.invalid/v1");
  } finally {
    if (originalAgentCmd === undefined) delete process.env["AGENT_CMD"];
    else process.env["AGENT_CMD"] = originalAgentCmd;
