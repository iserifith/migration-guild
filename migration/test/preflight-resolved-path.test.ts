...
const CREDENTIAL_ENV = "EXAMPLE_PRIVATE_API_KEY";
...
const MAPPING_CASES: Array<{
  ...
  },
  {
    ...
    response: { status: 400, body: { error: { message: "The model `pvt/hy3-tencent` does not exist" } } },
    verdict: "fail",
    stage: "model-availability",
  ...
  {
    name: "network error",
    response: { networkError: "ECONNREFUSED example-private.invalid:443" },
    verdict: "fail",
    stage: "response",
    reason: /ECONNREFUSED/,
  },
...
for (const testCase of MAPPING_CASES) {
  test(`preflight maps ${testCase.name} to ${testCase.stage ?? "a pass"}`, async () => {
    ...
    assert.match(calls[0].url, /^https:\/\/example-private.invalid\/v1\//);
    ...
    assert.equal(result.resolved?.providerBaseUrl, "https://example-private.invalid/v1");
...
test("divergences are reported even when the live check succeeds", async () => {
  ...
  assert.equal(divergence.declaredValue, "https://example-private.invalid/v1");
  ...
  assert.deepEqual(json.divergences, [
    { setting: "model.base_url", declared: "https://example-private.invalid/v1", resolved: "https://ambient.example/v1" },
  ]);
...
function stubVerdict(verdict: PreflightResult["verdict"], failedStage?: PreflightStageId): PreflightResult {
  return {
    ...
    resolved: {
      ...
      providerBaseUrl: "https://example-private.invalid/v1",
...
test("T044: a reachable provider that answers with an unparseable body is distinguished from one that is unreachable", async () => {
  ...
  const unreachable = await preflight([{ networkError: "ECONNREFUSED example-private.invalid:443" }]);
