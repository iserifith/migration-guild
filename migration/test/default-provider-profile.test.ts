...
test("US3 (c): the seeded config carries no private-relay/pvt/DashScope default-provider references (SC-003)", () => {
  const serialized = JSON.stringify(DEFAULT_GUILD_CONFIG).toLowerCase();
  for (const banned of ["dashscope", "pvt/", "private-relay"]) {
    assert.equal(
  ...
  for (const [key, agent] of Object.entries(DEFAULT_GUILD_CONFIG.agents)) {
    ...
    assert.equal(
      String(model).startsWith("pvt/"),
      false,
      `agents.${key}.model must not carry a private-relay namespace`,
    );
  }
});
