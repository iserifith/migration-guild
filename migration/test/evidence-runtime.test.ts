...
test("runVerify scrubs secret env vars from subprocesses and redacts leaked values", async () => {
  ...
  const previous = process.env.EXAMPLE_PRIVATE_API_KEY;
  process.env.EXAMPLE_PRIVATE_API_KEY = "secret-value-for-redaction";
  try {
    const operatorToken = seed(db);
    const result = await runVerify(db, {
      artifactId: ARTIFACT_ID,
      ...
      operatorToken,
      commands: [
        "node -e \"console.log(process.env.EXAMPLE_PRIVATE_API_KEY || 'missing'); console.error('secret-value-for-redaction')\"",
      ],
      outputDir: path.join(workspace, ".guild", "evidence"),
    });
    ...
    if (previous === undefined) delete process.env.EXAMPLE_PRIVATE_API_KEY;
    else process.env.EXAMPLE_PRIVATE_API_KEY = previous;
    db.close();
    fs.rmSync(workspace, { recursive: true, force: true });
  }
});
