...
export function useArtifacts(query: ArtifactQuery = {}): UseArtifactsResult {
  ...
  // ⚡ Bolt: Memoize the hook return object to prevent referential instability and cascading re-renders in child components
  return useMemo(
    () => ({
...
export function useSociety(artifactId?: string): UseSocietyResult {
  ...
  // ⚡ Bolt: Memoize the hook return object to prevent referential instability and cascading re-renders in child components
  return useMemo(
    () => ({ society: state.data, loading: state.loading, error: state.error, reload: state.reload }),
    [state.data, state.loading, state.error, state.reload]
  );
}
