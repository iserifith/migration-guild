...
export function useArtifacts(query: ArtifactQuery = {}): UseArtifactsResult {
  ...
  // ⚡ Bolt: Wrap intermediate hook returns in useMemo using granular state dependencies.
  // This referentially stabilizes the object literal, preventing unnecessary cascading
  // re-renders in memoized child components when fetched data hasn't structurally changed.
  return useMemo(() => ({
