...
type BatchValidationClient = Pick<ProviderClient, "uploadFile" | "submitBatchJob" | "cancelBatchJob">;
...
export async function validateBatchSupport(
  ...
  try {
    ...
    if (message.includes("invalid_deployment_type")) {
      throw new Error(
        `[guildctl] Provider batch preflight failed for model "${model}". ` +
          "This deployment is not batch-capable. Use a deployment with a supported batch SKU " +
          "(for example globalbatch or datazonebatch), disable batch for this phase, or switch the phase provider to agent."
      );
    }
    throw err;
  }
...
export async function submitBatch(
  ...
  db.prepare(
    `INSERT INTO batch_jobs (provider_job_id, type, wave, artifact_ids)
     VALUES (?, ?, ?, ?)`,
  ).run(batchResponse.id, type, wave ?? null, JSON.stringify(usedArtifactIds));

  return db
    .prepare(`SELECT * FROM batch_jobs WHERE provider_job_id = ?`)
