...
- Q: How does ingestion actually populate the documentation index — a deterministic scraper/parser pipeline, or something else? → A: A harness-driven ingestion agent (run through the same harness as Migrate/Critic) uses whatever web-fetch/browsing tools its harness CLI already provides natively (e.g., Claude Code's WebFetch/WebSearch, or shell+curl) to gather source material for each locked library/version, rather than a bespoke Maven-artifact-classifier scraper built from scratch.
- Q: Must every documentation entry carry a verifiable source to guard against ingestion-time hallucination? → A: Yes. Every Documentation Entry MUST record the source URL and a verbatim excerpt it was derived from; an entry without provenance is not indexed. This keeps ingestion consistent with the project's "Evidence Over Assertion" constitutional principle — an entry can be spot-checked or re-fetched, not just trusted.
- Q: Should the new internal MCP server (FR-009) also expose browsing/fetch tools, or be scoped to index-query operations only? → A: Scoped to index-query tools only (`lookup_library_doc`, `verify_library_docs`). Ingestion does not go through this MCP server at all — it runs as a harness-driven agent using that harness's own existing web tools, keeping the new MCP surface smaller and avoiding rebuilding browsing capability several harnesses already ship.
...
**Independent Test**: Run ingestion (the harness-driven ingestion agent, using its harness CLI's native web tools) against a workspace with a confirmed locked dependency set of 3 kept libraries at specific versions; confirm the index afterward contains documentation scoped to exactly those 3 libraries at exactly those versions, that every written entry carries a source URL and verbatim excerpt, and confirm re-running ingestion after a version lock changes updates the index rather than leaving stale entries queryable.

**Acceptance Scenarios**:

1. **Given** a workspace with a confirmed locked dependency set, **When** the ingestion agent runs, **Then** the index is populated with documentation for every kept library at its exact locked version, each entry carries a source URL and verbatim excerpt, and the ingestion result reports which libraries succeeded, failed, or were skipped (e.g., no citable documentation source found).
...
- **FR-003**: The system MUST provide an ingestion process — a harness-driven ingestion agent using its harness CLI's own existing web-fetch/browsing tools, not a bespoke scraper — that populates the index from the workspace's confirmed locked dependency set (per spec 006), scoped to exactly the kept libraries and their exact locked versions, and MUST populate the FTS5 index as part of the same ingestion pass.
- **FR-003a**: Every Documentation Entry the ingestion agent writes MUST carry a verifiable source: the URL it was retrieved from and a verbatim excerpt it was derived from. An entry lacking this provenance MUST NOT be written to the index — an ingestion-time claim without a checkable source is exactly the kind of self-report the project's evidence principle exists to reject.
...
- **FR-009**: Because no agent tool-registration surface currently exists in the codebase, this feature MUST stand up a standardized tool surface by implementing an internal MCP (Model Context Protocol) server exposing the index-query tools — `lookup_library_doc` and `verify_library_docs` — over a stdio interface, rather than a one-off adapter bolted onto the existing harness/CLI-adapter layer. This is a deliberate commitment, not a placeholder: it gives every current and future agent framework a single, protocol-standard way to call these tools instead of each framework needing its own throwaway integration. This MCP server is scoped to index queries only; it does NOT expose browsing/web-fetch tools (see FR-003, FR-014a).
...
- **FR-014a**: The ingestion agent MUST rely on the web-fetch/browsing tools its harness CLI already provides natively rather than the feature building and maintaining its own browsing tool; the ingestion agent is otherwise launched and run the same way any other harness-driven agent is (persona, non-interactive invocation, harness-appropriate tool permissions).
...
- **Documentation Entry**: A single indexed unit — library, exact version, class/method/signature, the documentation content (description, parameters, return type/behavior) needed to verify or inform a code reference, and its required provenance (source URL and verbatim excerpt it was derived from — FR-003a).
- **Ingestion Agent**: A harness-driven agent (run through the same harness/CLI-adapter layer as Migrate and Critic) responsible for populating the index for a workspace's locked dependency set, using its harness CLI's own native web-fetch/browsing tools rather than a bespoke scraper (FR-003, FR-014a).
...
- **SC-009**: 100% of Documentation Entries in the index carry a resolvable source URL and a verbatim excerpt; zero entries are written without provenance, checked across every ingestion run in testing.
...
- No agent tool-registration surface (e.g., a `registerTool`/`allowedTools` framework or MCP-server tool exposure) exists in the codebase today; agents currently run through a harness/CLI-adapter layer (`migration/guildctl/harness.ts`, driving `opencode`, `goose`, `codex`, or `copilot` per run). This spec commits to closing that gap with a standard internal MCP server over stdio (FR-009), scoped narrowly to the index-query tools (`lookup_library_doc`, `verify_library_docs`) — not to browsing/fetch tools, which the ingestion agent gets instead from whatever web tools its harness CLI already ships natively (FR-014a). This split is deliberate: it keeps the net-new MCP surface as small as possible given this is genuinely new infrastructure for the repo, and avoids rebuilding browsing capability some harnesses (e.g., Claude Code's WebFetch/WebSearch) already provide. MCP-client feasibility for the query tools is confirmed, not merely plausible: all three of `opencode`, `goose`, and `codex` — the harnesses `harness.ts` resolves today — are MCP-client-capable via a non-interactive, config-file-driven mechanism (opencode's `mcp` block in `opencode.json`; goose's `extensions` block in `config.yaml` with `type: stdio`, documented for headless/scripted setups; codex's `[mcp_servers.<name>]` in `config.toml`, settable via the same `-c key=value` override style `codex.mjs` already uses for provider config). None of the three existing adapter scripts (`package/harness/opencode.mjs`, `codex.mjs`, `goose.mjs`) wire this in today, so doing so for each is real, in-scope adapter work under FR-009 — not a discovered blocker. The fourth harness, `copilot` (`agent-shim.mjs`), was not checked for MCP or native web-tool support as part of this spec.
- The ingestion agent's specific harness/CLI choice (and therefore which native web tools it has available) is a plan-phase decision, not fixed by this spec; FR-014a only requires that ingestion use whatever its chosen harness already provides rather than a new bespoke browsing tool. A harness with no native web tool (if any of the four turn out to lack one) would need that gap addressed in planning — this spec does not assume every harness qualifies.
