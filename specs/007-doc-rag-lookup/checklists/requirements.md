...
- `/speckit-clarify` session (2026-08-16): resolved the previously-unstated ingestion mechanism through a multi-round discussion. Landed on: ingestion is a harness-driven agent using its harness CLI's own native web tools rather than a bespoke scraper (FR-003, FR-014a); every entry it writes requires a source URL + verbatim excerpt (FR-003a, SC-009), consistent with the project constitution's "Evidence Over Assertion" principle; and the new MCP server (FR-009) is scoped to index-query tools only, not browsing, to keep the net-new infrastructure smaller and avoid duplicating web tools some harnesses already ship. All checklist items still pass with these changes — no regressions, still behavior/outcome-focused rather than prescribing internal implementation.
