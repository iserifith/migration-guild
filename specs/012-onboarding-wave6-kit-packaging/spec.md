...
A user completes setup and `guildctl init` in a fresh workspace, opens `.guild/config.yaml`, and finds a default model profile whose provider endpoint and credential env var match what root `.env.example` and GETTING-STARTED.md document as the default (DashScope, `DASHSCOPE_API_KEY`) — or, if the repo chooses to keep rootsys as the default, the docs explicitly name rootsys as the default with `ROOTSYS_API_KEY`. Either direction is acceptable; the mismatch itself is the defect.
...
- **FR-006**: The default provider profile seeded by `guildctl init` (`DEFAULT_GUILD_CONFIG.model` and `profiles.default` in `migration/guildctl/config.ts`) MUST match the provider documented as default in root `.env.example` — DashScope (`DASHSCOPE_API_KEY`, `https://dashscope-intl.aliyuncs.com/compatible-mode/v1`). The DashScope direction is chosen (see Assumptions); the rootsys direction is rejected.
- **FR-007**: All existing tests asserting the current rootsys defaults (including `migration/test/auto-canary.test.ts` around lines 637-798 and any test asserting `DEFAULT_GUILD_CONFIG.model.base_url` or `profiles.default`) MUST be updated in the same change as FR-006, keeping the suite green.
...
- **Direction choice for FR-006**: DashScope is chosen. Root `.env.example` already declares DashScope "default for Migration Guild"; provider-neutrality (constitution VII) favors a documented public endpoint over an undocumented personal one; the `dashscope` profile already exists at `config.ts:82`, so `profiles.default` adopts its values. The rootsys endpoint remains available to users who explicitly configure it, but is no longer what a fresh init seeds.
