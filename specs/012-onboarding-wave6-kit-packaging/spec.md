...
- What happens to the tarball size when `package-lock.json` is included? It is approximately 71 KB of text (`migration/package-lock.json` is 73,030 bytes); negligible against the dist trees it ships beside.
...
- Direction choice: The issue offered two directions (make init's default match the documented DashScope default, or update the docs to match the example-private default). The maintainers chose a **third direction on 2026-08-19: generic OpenAI-compatible** — seeded defaults point at `https://api.openai.com/v1` with `OPENAI_API_KEY`, and **DashScope references are removed from shipped artifacts entirely**. This supersedes both issue directions: example-private was never documented anywhere, and DashScope (a specific named provider) is no longer the shipped default either. Provider-neutrality (constitution VII) is served best by a generic OpenAI-compatible default that the user repoints at their own endpoint by editing one base_url.
...
- **FR-006**: The default provider profile seeded by `guildctl init` (`DEFAULT_GUILD_CONFIG.model` and `profiles.default` in `migration/guildctl/config.ts`) MUST be **generic OpenAI-compatible**: `base_url` `https://api.openai.com/v1`, `api_key_env` `OPENAI_API_KEY`, and an OpenAI-compatible model identifier — the same neutral default the shipped docs name. The seeded `provider.routes` MUST drop example-private-namespace `pvt/*` identifiers so the seeded config is internally consistent with the OpenAI-compatible default. The seeded `agents.*.model` references (`deepseek-v4-pro`, `deepseek-v4-flash`, `glm-5.1`) do not carry a `pvt/`/example-private namespace today, but are not OpenAI-compatible model ids either — these MUST be replaced with OpenAI-compatible model identifiers, internally consistent with `model.model`. DashScope references MUST NOT appear in the seeded config: the `dashscope` profile and the dashscope-intl-pointing `cheap`/`reviewer`/`qwen` profiles are removed or repointed to OpenAI-compatible defaults.
- **FR-007**: All existing tests asserting the current example-private/DashScope defaults MUST be updated in the same change as FR-006, keeping the suite green. The update surface is wider than the issue's citation: a case-insensitive grep for `DEFAULT_GUILD_CONFIG|example-private|EXAMPLE_PRIVATE|pvt/|dashscope` over `migration/test/*.test.ts` matches **15 files (164 matches)**, of which **9 files need genuine edits** (literal-value assertions on the seeded example-private/DashScope defaults) and the remainder are either structural `DEFAULT_GUILD_CONFIG` usage with no value assertion, or use `EXAMPLE_PRIVATE_API_KEY`/`DASHSCOPE_API_KEY` only as an arbitrary secret-shaped literal in a redaction fixture, needing no change. `tasks.md`'s T011–T020 is the authoritative per-file breakdown (which files change, which stay green as-is, and why).
...
- **Tarball size**: the increase from `package-lock.json` (approximately 71 KB text) is negligible.
