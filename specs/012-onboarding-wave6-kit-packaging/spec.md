...
A user completes setup and `guildctl init` in a fresh workspace, opens `.guild/config.yaml`, and finds a default model profile whose provider endpoint and credential env var match what root `.env.example` and GETTING-STARTED.md document as the default (DashScope, `DASHSCOPE_API_KEY`) — or, if the repo chooses to keep private-relay as the default, the docs explicitly name private-relay as the default with `EXAMPLE_PRIVATE_API_KEY`. Either direction is acceptable; the mismatch itself is the defect.
...
1. **Given** a fresh workspace and the shipped `.env.example` (documenting `DASHSCOPE_API_KEY` as the default credential), **When** `guildctl init` generates `.guild/config.yaml`, **Then** the seeded default profile's `api_key_env` is `DASHSCOPE_API_KEY` (with `base_url` `https://dashscope-intl.aliyuncs.com/compatible-mode/v1`) — OR the docs are updated so private-relay is explicitly named as the seeded default with its credential var; the two artifacts must not contradict each other.
...
3. **Given** existing tests that assert on `DEFAULT_GUILD_CONFIG`'s current seeded values (e.g. `migration/test/auto-canary.test.ts:637-798` asserting `AGENT_PROVIDER_BASE_URL === "https://example-private.invalid/v1"` and `EXAMPLE_PRIVATE_API_KEY` propagation), **When** the default changes, **Then** those tests are updated in the same change so the suite stays green — config.ts is shared runtime code, and its seeded defaults are behavior under test.
4. **Given** config profiles beyond the default (dashscope, cheap, reviewer, qwen, local at `config.ts:82-86`), **When** this story lands, **Then** those named profiles remain available and unchanged (they are already correctly DashScope-consistent).

**Edge Cases**:

- Direction choice: The issue allows "make init's default match the documented DashScope default, or update the docs to match the real shipped default". The DashScope direction is preferred (root `.env.example` already declares DashScope "default for Migration Guild"; provider-neutrality per constitution VII favors a documented public endpoint over an undocumented personal one; and the dashscope profile already exists at `config.ts:82` so `profiles.default` simply adopts its values). The private-relay direction would additionally require re-writing `.env.example` and GETTING-STARTED.md provider docs and would leave doctor failing on an undocumented credential for every doc-following user — strictly worse.
...
- **FR-006**: The default provider profile seeded by `guildctl init` (`DEFAULT_GUILD_CONFIG.model` and `profiles.default` in `migration/guildctl/config.ts`) MUST match the provider documented as default in root `.env.example` — DashScope (`DASHSCOPE_API_KEY`, `https://dashscope-intl.aliyuncs.com/compatible-mode/v1`). The DashScope direction is chosen (see Assumptions); the private-relay direction is rejected.
- **FR-007**: All existing tests asserting the current private-relay defaults (including `migration/test/auto-canary.test.ts` around lines 637-798 and any test asserting `DEFAULT_GUILD_CONFIG.model.base_url` or `profiles.default`) MUST be updated in the same change as FR-006, keeping the suite green.
...
- **SC-003**: A fresh `guildctl init` seeds a default model profile consistent with the shipped docs; docs↔config consistency is enforced by a test comparing `DEFAULT_GUILD_CONFIG`'s default profile against the provider named in `.env.example`.
...
- **Direction choice for FR-006**: DashScope is chosen. Root `.env.example` already declares DashScope "default for Migration Guild"; provider-neutrality (constitution VII) favors a documented public endpoint over an undocumented personal one; the `dashscope` profile already exists at `config.ts:82`, so `profiles.default` adopts its values. The private-relay endpoint remains available to users who explicitly configure it, but is no longer what a fresh init seeds.
...
- **Out of scope, recorded for future waves**: (a) the `.guild/.env.example` written by `scaffoldGuildConfig` (config.ts:390) seeds `OPENROUTER_API_KEY`/`OPENAI_API_KEY`/`ANTHROPIC_API_KEY` — a third variant inconsistent with both root `.env.example` and the DashScope default; (b) `migration/guildctl/commands/benchmark.ts:92` legitimately still mentions `guildctl.config.json` as a legacy artifact in a migration warning — that mention stays.
