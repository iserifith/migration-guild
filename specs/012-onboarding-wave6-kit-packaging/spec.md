...
- **FR-006**: The default provider profile seeded by `guildctl init` (`DEFAULT_GUILD_CONFIG.model` and `profiles.default` in `migration/guildctl/config.ts`) MUST be **generic OpenAI-compatible**: `base_url` `https://api.openai.com/v1`, `api_key_env` `OPENAI_API_KEY`, and an OpenAI-compatible model identifier — the same neutral default the shipped docs name. The seeded `provider.routes` MUST drop rootsys-namespace `fiq/*` identifiers so the seeded config is internally consistent with the OpenAI-compatible default. The seeded `agents.*.model` references (`deepseek-v4-pro`, `deepseek-v4-flash`, `glm-5.1`) do not carry a `fiq/`/rootsys namespace today, but are not OpenAI-compatible model ids either — these MUST be replaced with OpenAI-compatible model identifiers, internally consistent with `model.model`. DashScope references MUST NOT appear in the seeded config: the `dashscope` profile and the dashscope-intl-pointing `cheap`/`reviewer`/`qwen` profiles are removed or repointed to OpenAI-compatible defaults.
- **FR-007**: All existing tests asserting the current rootsys/DashScope defaults MUST be updated in the same change as FR-006, keeping the suite green. The update surface is wider than the issue's citation: a case-insensitive grep for `DEFAULT_GUILD_CONFIG|rootsys|ROOTSYS|fiq/|dashscope` over `migration/test/*.test.ts` matches **15 files (164 matches)**, of which **9 files need genuine edits** (literal-value assertions on the seeded rootsys/DashScope defaults) and the remainder are either structural `DEFAULT_GUILD_CONFIG` usage with no value assertion, or use `ROOTSYS_API_KEY`/`DASHSCOPE_API_KEY` only as an arbitrary secret-shaped literal in a redaction fixture, needing no change. `tasks.md`'s T011–T020 is the authoritative per-file breakdown (which files change, which stay green as-is, and why).
