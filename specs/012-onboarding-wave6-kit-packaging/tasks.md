...
- [x] T011 [P] [US3] Update `migration/test/provider-routing.test.ts` (lines ~11, 13–24, 30–36): replace `EXAMPLE_PRIVATE_API_KEY`/`pvt/*` literal assertions (`api_key_env`, the three `resolveProviderRoute` route arrays, the `preflightProviderCredential`/`redactConfigForDisplay` credential-env literal) with the new `OPENAI_API_KEY`/OpenAI-compatible-id equivalents. Depends on T010.
- [x] T012 [P] [US3] Update `migration/test/guild-config-openai-compatible.test.ts` (lines 21, 38, 52): `cfg.model.base_url === "https://example-private.invalid/v1"` (line 21) and the YAML round-trip equivalent (line 38) → `"https://api.openai.com/v1"`; `delete process.env.EXAMPLE_PRIVATE_API_KEY;` (line 52) → `delete process.env.OPENAI_API_KEY;` (this credential-env cleanup line was not itemized in `plan.md`'s per-line list but is in the same file/task as lines 21/38). Depends on T010.
...
- [x] T014 [P] [US3] Update `migration/test/preflight-resolved-path.test.ts`: `CREDENTIAL_ENV = "EXAMPLE_PRIVATE_API_KEY"` (line 39) and all dependent fixture URLs/models throughout the file (e.g. lines ~122, 129, 193, 348, 353, 531, 656) → OpenAI-compatible equivalents. Depends on T010.
- [x] T015 [P] [US3] Update `migration/test/runtime-resolution.test.ts`: `EXAMPLE_PRIVATE_API_KEY` env fixtures throughout the file (e.g. lines ~44, 64, 90, 97, 109, 131, 156, 161, 183–210, 233–277, 371–443) and the `pvt/grok-4.5` model-divergence fixture (lines ~132, 138) → equivalents. Depends on T010.
- [x] T016 [P] [US3] Update `migration/test/auto-canary.test.ts`: `EXAMPLE_PRIVATE_API_KEY`/`example-private.invalid` propagation assertions throughout (e.g. lines ~638, 648, 654, 764–850) → `OPENAI_API_KEY`/`api.openai.com` equivalents. Depends on T010.
