...
- [x] T019 [P] [US3] Update `migration/test/workspace-isolation-defaults.test.ts` (line 109): `DASHSCOPE_API_KEY: "dummy"` env fixture → `OPENAI_API_KEY: "dummy"` — identical doctor-credential-fixture pattern to T017/T018, found during this task list's re-verification of `plan.md`'s FR-007 grep surface (`plan.md` cited only 13 files; this repo's current `grep -lE 'DEFAULT_GUILD_CONFIG|example-private|EXAMPLE_PRIVATE|pvt/|dashscope' migration/test/*.test.ts` matches this 14th file too). Depends on T010.
