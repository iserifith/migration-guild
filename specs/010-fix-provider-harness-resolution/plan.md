...
- **US2 / #125** — `resolveHarness()` in `migration/guildctl/harness.ts` (lines 19–38) reads `config.harness || "opencode"` and never reads `GUILDCTL_HARNESS`. Fix: precedence `GUILDCTL_HARNESS > config.harness > "opencode"`, with the env-sourced harness reported as a harness divergence whose `source` is derived via the existing `originOf()`/`envOrigin` path (the harness-divergence block in `resolveAgentLaunch()`, lines 142–145; `ConfigDivergenceSource` is NOT widened), and an unknown value throwing the existing named error (line 37).
...
**Change — environment divergence (lines 142–145)**: `resolveAgentLaunch()` already pushes a `harness` divergence when `harness.name !== declaredHarness` (line 143), reading `source: originOf("AGENT_CMD")` — but `originOf` only keys `AGENT_PROVIDER_BASE_URL`/`AGENT_CMD`. When the env source is `GUILDCTL_HARNESS`, derive the divergence `source` via the **existing** `originOf()`/`envOrigin` path. Do NOT widen `ConfigDivergenceSource` (`"ambient" | "project-file" | "config"`, harness.ts line 45). The divergence is keyed on `env.GUILDCTL_HARNESS` driving resolution — **distinct from** the `AGENT_CMD` case — so it must NOT flip the existing passing assertion `divergence.source === "project-file"` in `runtime-resolution.test.ts:409` (which supplies `origin: { AGENT_CMD: "project-file" }`); set the GUILDCTL_HARNESS divergence's source through the same origin map (e.g. `originOf("GUILDCTL_HARNESS")`) so the two are disambiguated before the source is chosen. `HarnessResolution.source` (harness.ts line 10) is a *different* field (where the harness choice came from) and already admits `"environment"` — that is the field the prior analysis confused with `ConfigDivergence.source`.
...
**Current behavior**: `checkHarness()` (harness.ts lines 166–180) IS already invoked for the **config-sourced** harness: `guildctl doctor` delegates to `runPreflight` (cli.ts lines 212–221), whose resolution stage calls `opts.checkAdapter ?? checkHarness` (preflight.ts lines 180–183) before the offline branch, so a missing config-sourced harness CLI is already caught for `doctor --offline`. The genuine remaining gaps for #126 are: (a) the **AGENT_CMD / custom-harness** case — preflight's resolution stage deliberately skips the check when `resolution.harness.source !== "config"` (the `if (resolution.harness.source === "config")` guard at preflight.ts:181 excludes it), so a custom-harness CLI absence is not caught at doctor; and (b) the **phase-launch boundary** (FR-008), where the launch path (auto.ts line 395 spawn, line 440 throw) does not consult any harness-availability result.

**Change A — doctor invokes `checkHarness` for the custom/AGENT_CMD case (doctor.ts, BEFORE the tableExists early-return at line 73–76)**: Import `resolveAgentLaunch` (or `resolveHarness`) and `checkHarness` from `./harness`. At the **top of `runPipelineStateChecks`, before line 60 / above the `tableExists` early-return (lines 73–76)** so a fresh workspace (no registry db yet) is still covered, resolve the harness for the configured workspace (`resolveAgentLaunch({ config: resolveGuildConfig({ cwd: workspaceRoot }), root: workspaceRoot })`), then call `checkHarness(resolution.harness)`. Push a `CheckResult`:
...
**Change B — launch path consults the result (runner.ts / commands/auto.ts)**: `spawnAgent` (runner.ts line 390) already resolves `launch = opts.resolution ?? resolveAgentLaunch(...)`; the actual harness `spawn` happens much later at runner.ts line 541. Before spawning (in `spawnAgent`, around runner.ts lines 390–391, well before the spawn call at :541), consult `checkHarness(launch.harness)` once. If `!ok`, throw a clear `active harness: <name> (<command> missing or unreachable)` error at the boundary instead of spawning and later failing with a cryptic exit code. This reuses the same function preflight uses (constitution I: single source of truth), so the boundary check and the doctor check cannot disagree. The phase-launch path should not spawn a new `--version` probe per artifact — it uses the already-resolved `launch.harness` and a single `checkHarness` call.
...
- New case: `resolveHarness({ ...config, harness: "codex" }, root, { GUILDCTL_HARNESS: "opencode" })` → `name === "opencode"` (env wins).
...
**Regression guard (SC-006)**: existing cases in `runtime-resolution.test.ts`, `env-precedence.test.ts`, `preflight-resolved-path.test.ts`, `doctor-pipeline-state.test.ts`, and `harness-selection.test.ts` (which directly tests `resolveHarness()` + `checkHarness()`, incl. the AGENT_CMD divergence source at line 409 and the "doctor harness check flags a missing selected command" case) must stay green; the precedence/divergence/error-surfacing changes are additive (new divergence field, new precedence branch, new failure-message content) and do not alter the existing `opencode default` / `AGENT_CMD` / `pass-on-valid-body` assertions.
...
- Length cap for captured stderr / raw-body excerpt: pinned at **512 chars** for both the raw-body excerpt (`redactCredential(bodyText.slice(0,512), credentialValue)`, preflight.ts lines 101–105) and the harness stderr surface (`stdoutStderr.slice(0,512)`), matching the existing `truncate` usage. No open question remains.
...
- US2 (#125): `GUILDCTL_HARNESS` precedence + environment-keyed divergence + unknown-value throw.
- US3 (#126, partial): `doctor` invokes `checkHarness` for the custom/AGENT_CMD case as a blocking finding (config-sourced case already covered by preflight delegation); launch-path boundary check.
- US4 (#120): shape tolerance + redacted raw-body excerpt on parse failure.
- Regression tests: `env-precedence.test.ts`, `runtime-resolution.test.ts`, `preflight-resolved-path.test.ts`, `doctor-pipeline-state.test.ts`, `harness-selection.test.ts` extended.
- SC-001..SC-004, SC-006. (FR-013 docs MUST is relaxed to SHOULD-for-MVP — see below; SC-007 is verified by maintainer read-through, no automated gate, so it is not a hard MVP gate.)
...
- Docs pass: `HARNESS.md` + `GETTING-STARTED.md` updates (FR-013, SC-007) documenting corrected `GUILDCTL_HARNESS` precedence, empty-vs-unset credential behavior, and installed-harness-CLI requirement. FR-013 is written as a MUST in the spec but is intentionally delivered as the Incremental/docs step (SHOULD-for-MVP milestone); pulling the docs into MVP is permitted but not required for the MVP approval gate.
- Maintainer read-through verification (SC-007) — manual, post-MVP.
