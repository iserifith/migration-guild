...
- `migration/guildctl/env.ts` — `loadGuildEnvironment()` applies a workspace `.env` value only when `value != null` (the `if (fileWins)` block at line 196, with `target[key] = value` at line 197). An empty `.env` value (`NINE_ROUTER_API_KEY=`) is a defined key with a falsy value, and because the workspace file is spread into `fileValues` via `const fileValues = { ...project }` at line 157 (where `project` was parsed at line 154), that empty string enters `fileValues` and overwrites a working ambient value at line 197. Divergence reporting (`env.ts` §3, lines 174–187) only compares declared vs ambient and never flags "empty-but-defined". This is the #119 trap.
- `migration/guildctl/harness.ts` — `resolveHarness()` (lines 19–38) reads `config.harness || "opencode"` and never reads `GUILDCTL_HARNESS`. `config.harness` is always a string in `GuildConfig` (config.ts line 10), so it is never falsy/empty — `env.GUILDCTL_HARNESS` can never win. This is the #125 bug. `checkHarness()` (lines 166–180) exists and is already invoked by preflight's resolution stage for the **config-sourced** harness (`if (resolution.harness.source === "config")`, preflight.ts lines 180–183), which fires before the offline branch so it also covers `guildctl doctor --offline` (cli.ts:212-221 → runPreflight). The genuine remaining gaps for #126 are (a) the AGENT_CMD/custom-harness case that preflight's resolution stage deliberately skips (`source === "config"` excludes it), and (b) the phase-launch boundary (FR-008), where the launch path does not yet consult any harness-availability result. The harness-divergence block in `resolveAgentLaunch()` (lines 142–145) is unaffected by this.
...
- **FR-005**: A harness resolved from `GUILDCTL_HARNESS` MUST be reported as a harness divergence in `resolveAgentLaunch()` whose `source` is derived correctly via the existing `originOf()`/`envOrigin` path, mirroring the existing `AGENT_CMD` divergence pattern in `migration/test/runtime-resolution.test.ts`. The divergence is keyed on `env.GUILDCTL_HARNESS` driving resolution — **distinct from** the `AGENT_CMD` case — and `ConfigDivergenceSource` (`"ambient" | "project-file" | "config"`, harness.ts line 45) is explicitly NOT widened. The natural keying (`harness.source === "environment"`) MUST NOT flip the existing assertion `divergence.source === "project-file"` in `runtime-resolution.test.ts:409`, which assumes `origin: { AGENT_CMD: "project-file" }` is supplied; the GUILDCTL_HARNESS case must set its source via the same origin map (e.g. `originOf("GUILDCTL_HARNESS")`) so the two are disambiguated before the source is chosen. Precedence `GUILDCTL_HARNESS > config.harness > "opencode"` is unchanged (correct). (#125)
...
- **FR-013**: `HARNESS.md` and `GETTING-STARTED.md` MUST be updated to document the corrected `GUILDCTL_HARNESS` precedence, the empty-vs-unset `.env` credential behavior, and the requirement that the selected harness CLI be installed (or `doctor` will fail closed). (#125, #119, #126) — written as a MUST for the feature, but delivered in the Incremental/docs step (SHOULD-for-MVP milestone; the MVP approval gate does not require it).
...
- **SC-006**: `npm test` in `migration/` passes with the new regression tests; no existing test in `runtime-resolution.test.ts`, `env-precedence.test.ts`, `preflight-resolved-path.test.ts`, `doctor-pipeline-state.test.ts`, or `harness-selection.test.ts` (which directly tests `resolveHarness()`/`checkHarness()`, incl. the AGENT_CMD divergence `source === "project-file"` at line 409) is broken by the precedence/error-surfacing changes. (all stories)
- **SC-007**: `HARNESS.md` + `GETTING-STARTED.md` accurately describe the corrected `GUILDCTL_HARNESS` precedence, empty-vs-unset credential behavior, and the installed-harness-CLI requirement, verified by a maintainer read-through (no automated gate — consistent with the Assumption that docs are a manual-verify step). (Story 2, 1, 3)
