...
- The "working" recency threshold is a new, dashboard-specific constant, not a reuse of `guildctl doctor`'s 60-minute dangling-claim threshold — the two answer different questions ("actively progressing right now" vs. "probably abandoned"). Per issue #220 and issue #218 (specifying a supervisor staleness sweep concurrently, expected at `specs/015-supervisor-staleness-sweep`), these two thresholds (and possibly a third, if #218 introduces its own) conceptually overlap and should probably be reconciled into a shared constant or configuration surface at some point. That reconciliation is explicitly out of scope for this feature and is noted here as a future consideration, not resolved.
