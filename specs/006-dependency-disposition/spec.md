...
- What happens when a library is declared but never actually used by any in-scope artifact (dead declaration)? The planner should still emit a disposition for it (typically inline or an explicit no-keep outcome) rather than silently dropping it, so the record explains why it vanished from the target set.
