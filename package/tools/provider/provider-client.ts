...
export class ProviderClient {
  ...
  constructor(cfg: ProviderConfig, opts: ProviderClientOptions = {}) {
  ...
  async embedOne(text: string): Promise<number[]> {
    ...
    if (!first) throw new Error("[ProviderClient] Empty embedding response");
  ...
  async pollBatchJob(providerJobId: string): Promise<BatchJobResponse> {
    const url = `${this.openaiEndpoint}/batches/${providerJobId}`;
    return this.fetchWithRetry<BatchJobResponse>(url, { method: "GET" });
  }

  async cancelBatchJob(providerJobId: string): Promise<BatchJobResponse> {
    const url = `${this.openaiEndpoint}/batches/${providerJobId}/cancel`;
  ...
  async uploadFile(content: string, filename: string): Promise<{ id: string }> {
    ...
    if (!res.ok) {
      ...
      throw new Error(`[ProviderClient] File upload failed ${res.status}: ${text}`);
    }
  ...
  async downloadFile(fileId: string): Promise<string> {
    ...
    if (!res.ok) {
      const text = await res.text();
      throw new Error(`[ProviderClient] File download failed ${res.status}: ${text}`);
    }
    return res.text();
  }
  ...
  private async fetchWithRetry<T>(
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(`[ProviderClient] HTTP ${res.status} ${res.statusText}: ${body}`);
      }

      return res.json() as Promise<T>;
    } catch (err) {
      if ((err as Error).name === "AbortError") {
        throw new Error(`[ProviderClient] Request timed out after ${this.timeoutMs}ms: ${url}`);
      }
